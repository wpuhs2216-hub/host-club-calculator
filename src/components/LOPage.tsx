...
export const LOPage: React.FC<LOPageProps> = ({ tables, config, dispatchForSlip, onMoveSlip, onClearAllSlips }) => {
    ...
    const tableSummaries = useMemo(() => {
        return tables.map(table => {
            let totalCurrent = 0;
            let totalClosing = 0;
            for (const slip of table.slips) {
                const result = calculateResult(slip.state, config, { loCapEnabled: true });
                const closing = result.schedule[result.schedule.length - 1];
                totalCurrent += result.currentTotal;
                totalClosing += closing?.totalPrice ?? 0;
            }
            return { id: table.id, slipCount: table.slips.length, totalCurrent, totalClosing };
        });
    }, [tables, config]);
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-3 gap-2">
                {tables.map(table => {
                    const summary = tableSummaries.find(s => s.id === table.id)!;
                    const isExpanded = expandedTableId === table.id;
                    const hasSlips = summary.slipCount > 0;
                    ...
                    return (
                        <button
                            ...
                            className={`p-3 rounded-xl border text-left transition-all cursor-pointer ${
                                isExpanded
                                    ? 'border-[var(--gold-color)] bg-[rgba(255,215,0,0.1)] col-span-3'
                                    : hasSlips
                                        ? 'border-[var(--accent-color)] bg-[var(--card-bg)] hover:border-[var(--gold-color)]'
                                        : 'border-[var(--border-color)] bg-[var(--card-bg)] opacity-50'
                            }`}
                        >
                            <div className="text-base font-bold text-[var(--gold-color)]">{table.name}</div>
                            <div className="text-xs text-gray-400">{summary.slipCount}伝票</div>
                            {hasSlips && (
                                <div className="text-sm font-mono font-bold text-[var(--accent-color)] mt-1">
                                    ¥{(defaultMode === 'closing' ? summary.totalClosing : summary.totalCurrent).toLocaleString()}
                                </div>
                            )}
                            {!hasSlips && <div className="text-xs text-gray-600 mt-1">---</div>}
                        </button>
                    );
                })}
            </div>
            ...
            {/* 展開されたテーブルの伝票詳細 */}
            {expandedTableId && (() => {
                const table = tables.find(t => t.id === expandedTableId);
                if (!table || table.slips.length === 0) return (
                    <div className="p-4 text-center text-gray-500 text-sm rounded-xl border border-[var(--border-color)] bg-[var(--card-bg)]">
                        伝票なし
                    </div>
                );
